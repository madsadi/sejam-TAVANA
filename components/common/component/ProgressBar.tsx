...
export default function ProgressBar(props: ProgressBarProps) {
    ...
    const solutions = [
        { name: 'پروفایل', description: `${data?.result?.firstName + " " + data?.result?.lastName}`, href: '/profile', icon: ChartPieIcon },
    ]
    return (
                        ...
                        progress.map((step: any, index: number) => {
                            return (
                                <li suppressHydrationWarning={true}
                                    className={`relative md:justify-end justify-center items-center font-bold grow text-center bg-no-repeat bg-center p-2 md:h-[150px] h-[81px] ${level > step.level ? 'step-done' : (level === step.level ? (regInfo.registrationState === 18 && step.level === 6 ? 'step-done' : 'step-active shadow-[0_2px_0px_0px_#D9D9D9]') : 'step-done')}`}
