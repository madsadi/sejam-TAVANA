...
export default function SejamInfoLevel() {
  ...
  const { mutate: registerBankAccount } = useMutation({
    url: `${SEJAM_URL}/api/request/RegisterBankAccount`,
  });
  ...
  const setDefaultBank = async (defaultBank: accountNumber) => {
    ...
    await registerBankAccount({ bankAccounts: [...restOfAccounts] }).then(() =>
      bankAccounts()
    );
  };
  ...
  return (
    <>
      <div className="grow ">
        {data?.legalPerson ? (
          <AccordionComponent title={"اطلاعات هویتی حقوقی"}>
            <div className="grid md:grid-cols-4 grid-cols-2 gap-3">
              <LabelValue
                title={"نام شخص حقوقی"}
                value={data?.legalPerson?.companyName}
              />
              <LabelValue
                title={"شماره ثبت شخص حقوقی"}
                value={data?.legalPerson?.registerNumber}
              />
              <LabelValue
                title={" تاریخ ثبت"}
                value={jalali(data?.legalPerson?.registerDate).date}
              />
              <LabelValue
                title={"محل ثبت"}
                value={data?.legalPerson?.registerPlace}
              />
              <LabelValue
                title={"کد اقتصادی"}
                value={data?.legalPerson?.economicCode}
              />
              <LabelValue
              ...
              <LabelValue
                ...
                value={data?.legalPerson?.evidenceExpirationDate}
              />
              <LabelValue
                title={"نوع شرکت"}
                value={
                  legalPersonTypeCategoryEnums.find(
                    (item: any) =>
                      item.id === data?.legalPerson?.legalPersonTypeCategory
                  )?.title
                }
              />
              <LabelValue
                ...
                value={
                  legalPersonTypeSubCategory.find(
                    (item: any) =>
                      item.id === data?.legalPerson?.legalPersonTypeSubCategory
                  )?.title
                }
              />
            </div>
          ...
          <AccordionComponent title={"اطلاعات هویتی"}>
            <div className="grid md:grid-cols-4 grid-cols-2 gap-3">
              <LabelValue
                title={"نام"}
                value={data?.privatePerson?.firstName}
              />
              <LabelValue
                title={"نام خانوادگی"}
                value={data?.privatePerson?.lastName}
              />
              <LabelValue
                title={"تاریخ تولد"}
                value={
                  data?.privatePerson?.birthDate
                    ? jalali(data?.privatePerson?.birthDate).date
                    : ""
                }
              />
              <LabelValue
                title={"نام پدر"}
                value={data?.privatePerson?.fatherName}
              />
              <LabelValue
                title={"محل تولد"}
                value={data?.privatePerson?.placeOfBirth}
              />
              <LabelValue
                title={"صادره از"}
                value={data?.privatePerson?.placeOfIssue}
              />
              <LabelValue
                title={"سریال شناسنامه"}
                value={
                  data?.privatePerson?.serial
                    ? `${
                        data?.privatePerson?.serial +
                        `/` +
                        data?.privatePerson?.seriShChar +
                        data?.privatePerson?.seriSh
                      }`
                    : ""
                }
              />
              <LabelValue
                ...
                value={data?.privatePerson?.shNumber}
              />
            </div>
          </AccordionComponent>
        )}
        <AccordionComponent title={"اطلاعات شغلی"}>
          <div className="grid md:grid-cols-4 grid-cols-2  gap-3">
            ...
            <LabelValue
              title={"شماره شرکت"}
              value={
                data?.jobInfo?.companyPhone
                  ? `${
                      data?.jobInfo?.companyCityPrefix
                        ? data?.jobInfo?.companyCityPrefix + "-"
                        : "" + data?.jobInfo?.companyPhone
                    }`
                  : ""
            ...
            <LabelValue
              title={"کد پستی شرکت"}
              value={data?.jobInfo?.companyPostalCode}
            />
            <LabelValue
              title={"سایت شرکت"}
              value={data?.jobInfo?.companyWebSite}
            />
            <LabelValue
              title={"تاریخ استخدام"}
              value={
                data?.jobInfo?.employmentDate
                  ? jalali(data?.jobInfo?.employmentDate)?.date
                  : ""
              }
            />
        ...
        {data?.legalPersonShareholders?.length ? (
          <AccordionComponent title={"اطلاعات اعضای هیت مدیره"}>
            {data?.legalPersonShareholders?.map(
              (item: LegalPersonShareholderType) => {
                return (
                  <div
                  ...
                  >
                    ...
                    <LabelValue
                      title={"درصد سهامداری"}
                      value={`% ${item.percentageVotingRight}`}
                    />
                  </div>
                );
              }
        ...
        {data?.legalPersonStakeholders?.length ? (
          <AccordionComponent title={"اطلاعات ذی نفعان"}>
            {data?.legalPersonStakeholders?.map(
              (item: legalPersonStakeholdertype) => {
                return (
                  <div
                    className="grid md:grid-cols-4 grid-cols-2  gap-3"
                    key={item?.uniqueIdentifier}
        ...
        {data?.tradingCodes?.length && (
          <AccordionComponent title={"کد های بورسی"}>
        ...
        {data?.agent ? (
          <AccordionComponent title={"وکیل / نماینده"}>
            <div className="grid md:grid-cols-4 grid-cols-2  gap-3">
              <LabelValue
                title={"مشخص کننده نوع نماینده"}
                value={
                  agentTypeEnums.find(
                    (item: any) => item.id === data?.agent?.type
                  )?.title
                }
              />
              <LabelValue
                ...
                value={
                  data?.agent?.expirationDate
                    ? jalali(data?.agent?.expirationDate).date
                    : ""
                }
              />
              ...
              <LabelValue
                title={"کد ملی"}
                value={data?.agent?.uniqueIdentifier}
              />
              <LabelValue title={"نام"} value={data?.agent?.firstName} />
              <LabelValue
                title={"نام خانوادگی"}
                value={data?.agent?.lastName}
              />
              <LabelValue
              ...
              >
                ...
                <LabelValue
                  ...
                  value={
                    (item?.emergencyTelCityPrefix
                      ? item?.emergencyTelCityPrefix + "-"
                      : "") + (item?.emergencyTel ? item?.emergencyTel : "")
                  }
                />
                <LabelValue
                  key={index}
                ...
                <LabelValue
                  ...
                  value={
                    ...
                    " " +
                    (item?.remnantAddress ? item?.remnantAddress : "") +
                    " " +
        ...
        <AccordionComponent title={"اطلاعات مالی"}>
          <div className="grid md:grid-cols-4 grid-cols-2  gap-3">
            <LabelValue
              title={"ارزش دارایی"}
              value={formatNumber(data?.financialInfo?.assetsValue)}
            />
            <LabelValue
                ...
                data?.legalPerson
                  ? transactionLevelLegalPersonEnums.find(
                      (item: { id: string; title: string }) =>
                        item.id === data?.financialInfo?.transactionLevel
                    )?.title
            ...
            <LabelValue
              ...
              value={
                tradingKnowledgeLevelEnums.find(
                  (item: { id: string; title: string }) =>
                    item.id === data?.financialInfo?.tradingKnowledgeLevel
                )?.title
              }
            />
            <LabelValue
            ...
            <LabelValue
              ...
              value={data?.financialInfo?.rateDate}
            />
            <LabelValue
              title={"رتبه اخذ شده"}
              value={data?.financialInfo?.rate}
            />
            <div>
        ...
        <p className="font-semibold">
          چنانچه نسبت به تغییر آن حساسیتی دارید باید از طریق آن سامانه اقدام
          نمایید.
        </p>
      </div>
