...
import { numberInput, p2e } from "../../common/functions";
...
export default function ProfileSetter() {
  const { fetchAsyncData: searchCountry } = useQuery({
    url: `${SEJAM_URL}/api/request/SearchCountry`,
  });
  const { mutate: addCustomer } = useMutation({
    url: `${SEJAM_URL}/api/request/AddCustomerProfileInfo`,
  });
  const { setLevel, registrationState, regInfo } =
  ...
  const searchCountryHandler = async (e: any) => {
    ...
    await searchCountry({ CountryName: e.target.value }).then((res) =>
      setCountries(res?.data.result?.response)
    );
  };
  ...
  const returnCondition = (regInfo: any) => {
    if (
      ((regInfo.hasAgent && regInfo?.agentUniqueId) ||
        (!regInfo.hasAgent && !regInfo?.agentUniqueId)) &&
      regInfo.personType &&
      (regInfo.countryId === 1 ||
        (regInfo.countryId !== 1 && regInfo.foriegnCSDCode))
    ) {
      return true;
    } else {
      return false;
  ...
  return (
    <form className={"flex flex-col"} onSubmit={submitHandler}>
        ...
        <div className={"grid grid-cols-1 gap-4"}>
          <div className="flex items-center">
            <label htmlFor="confirm">
              آیا شما وکیل/نماینده شخص ثبت نامی در سامانه سجام هستید؟
            </label>
            <div
              className={
                "relative w-[100px] flex bg-white p-1 rounded-full h-full border-2 border-white shadow-[0_0_0_1px_#eee] mr-2 overflow-hidden"
              }
            >
              <div
                className={`bg-tavanaPurple rounded-full w-1/2 h-[90%] z-0 absolute right-0 top-1/2 -translate-y-1/2 transition-all ${
                  !info.hasAgent ? "-translate-x-full" : "translate-x-0"
                }`}
              />
              <button
                type={"button"}
                className={`grow text-center z-[1] ${
                  info.hasAgent ? "text-white" : ""
                }`}
                onClick={() => infoUpdate("hasAgent", true)}
              >
                بله
              </button>
              <button
                type={"button"}
                className={`grow text-center z-[1] pr-1 ${
                  !info.hasAgent ? "text-white" : ""
                }`}
                onClick={() => infoUpdate("hasAgent", false)}
              >
                خیر
              </button>
            </div>
          ...
          {info.hasAgent ? (
            <div
              className={
                "flex flex-col md:flex-row space-y-3 md:space-y-0 w-full"
              }
            >
              <label
                className={"flex items-center mb-1 ml-0 md:ml-3 min-w-[110px]"}
              >
                کد ملی وکیل:
              </label>
              <input
                ...
                onChange={(e) =>
                  infoUpdate(
                    "agentUniqueId",
                    numberInput(p2e(`${e.target.value}`))
                  )
                }
              />
            </div>
          ...
          <Listbox
            value={info.personType}
            onChange={(e) => infoUpdate("personType", e)}
          >
            {({ open }) => (
              <div
                className={
                  ...
                  <Listbox.Button className="relative input w-full bg-white py-1.5 pr-3 pl-10">
                    <span className="flex items-center">
                      <span className="ml-3 block truncate">
                        {
                          personType.find(
                            (item: any) => item.id === info.personType
                          )?.title
                        }
                      </span>
                    </span>
      ...
      <div
        className={
          "flex md:flex-row flex-col md:space-y-0 space-y-2 items-center space-x-2 space-x-reverse mr-auto mt-5"
        }
      >
        <button
          className={"button w-fit min-w-fit"}
